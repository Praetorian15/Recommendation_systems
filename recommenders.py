...
class MainRecommender:
    ...
    def get_similar_users_recommendation(self, user, N=5):
        ...
        users = [self.id_to_userid[rec[0]] for rec in model.similar_users(userid_to_id[user], #userid- id от 0 до N
                           user_items=csr_matrix(self.user_item_matrix).tocsr(),   # на вход user-item matrix
                           N=N, # кол-во рекомендаций 
                           filter_already_liked_items=False, 
                           filter_items=[itemid_to_id[999999]], 
                           recalculate_user=False)]
        popularity = data[data['user_id'].isin(users)].groupby('item_id')['quantity'].sum().reset_index()

        top_5 = popularity1.sort_values('quantity', ascending=False).head(N + 1).item_id.tolist()[1:]
